...
namespace FreeDraw
{
    ...
    public class Drawable : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            bool mouse_held_down = mouseButtonDown;
            if (mouse_held_down && !no_drawing_on_current_drag)
            {
                // Convert mouse coordinates to world coordinates
                Vector3 mouse_world_position = Camera.main.ScreenToWorldPoint(Mouse.current.position.ReadValue());

                Ray ray = Camera.main.ScreenPointToRay(Mouse.current.position.ReadValue());
                Debug.DrawRay(ray.origin, ray.direction * 1000, Color.red, 2f);

                // Check if the current mouse position overlaps our image
                //Debug.DrawRay(mouse_world_position, Vector3.forward * 100, Color.red, 1f);
